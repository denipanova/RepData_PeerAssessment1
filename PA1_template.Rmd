...
#cretae the table
for(i in 1:length(days)){
  mean_current = round(mean(data[data$date == days[i],]$steps,na.rm=TRUE),0)
  median_current = median(data[data$date == days[i],]$steps,na.rm=TRUE)
...
mm_table = cbind(days,mm_table)
colnames(mm_table) = c('day','mean','median')
mm_table$day = as.Date(mm_table$day)
print(mm_table)
...
```{r echo=TRUE}
plot(mm_table$day,mm_table$mean,type = 'l', col = 'blue',
     main = 'Time series plot of\n the average number of steps every day',
